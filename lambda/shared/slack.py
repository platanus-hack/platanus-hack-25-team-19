...
class SlackHelper:
    ...
    def check_for_user_reply(self, channel_id: str, message_ts: str, user_id: str) -> Optional[str]:
        ...
        try:
            ...
            print(f"[DEBUG] Got {len(replies)} replies for channel {channel_id}, ts {message_ts}")

            if len(replies) > 0:
                first_reply = replies[0]
                print(f"[DEBUG] First reply structure: {json.dumps(first_reply, indent=2)}")

                # Try to extract text from different possible locations in the message structure
                # 1. Try the 'text' field first (most common)
                if 'text' in first_reply and first_reply['text']:
                    print(f"[DEBUG] Found text in 'text' field: {first_reply['text']}")
                    return first_reply['text']

                # 2. Try to extract from blocks structure
                if 'blocks' in first_reply:
                    try:
                        text = first_reply["blocks"][0]["elements"][0]["elements"][0]["text"]
                        print(f"[DEBUG] Found text in blocks structure: {text}")
                        return text
                    except (KeyError, IndexError, TypeError) as e:
                        print(f"[DEBUG] Failed to extract from blocks structure: {e}")
                        pass

                # 3. If we can't find text, log the structure for debugging
                logger.warning(f"Could not extract text from reply structure: {json.dumps(first_reply)}")
                print("[DEBUG] Could not extract text from any known location")
                return None

            print(f"[DEBUG] No replies found for channel {channel_id}")
            return None

        except Exception as e:
            logger.error(f"Error checking for reply in {channel_id}: {e}")
            print(f"[DEBUG] Exception in check_for_user_reply: {e}")
            return None
